...
// EnsureMainCluster ensures a main KWOK cluster exists with the given name and API port.
// If it doesn't exist, it will be created and started. Returns the kubeconfig path.
func (km *KWOKClusterManager) EnsureMainCluster(name string, apiPort int) (string, error) {
	// Quick check: if directory exists, assume present and try to start
	clusterPath := filepath.Join(os.Getenv("HOME"), ".kwok", "clusters", name)
	kubeconfigPath := filepath.Join(clusterPath, "kubeconfig.yaml")
	if _, err := os.Stat(clusterPath); err == nil {
		// Attempt to start to ensure it's running
		startCmd := exec.Command(km.kwokctlPath, "start", "cluster", "--name", name)
		if out, serr := startCmd.CombinedOutput(); serr != nil {
			logrus.Debugf("EnsureMainCluster: start failed for %s: %v, out=%s", name, serr, string(out))
		} else {
			_ = km.waitForClusterReady(name)
		}

		// Validate that the API at apiPort is actually speaking HTTPS; if not, recreate with secure port.
		// Many earlier revisions created the main cluster with --kube-apiserver-insecure-port, which breaks Rancher
		// because it always dials HTTPS using the caCert.
		host := fmt.Sprintf("127.0.0.1:%d", apiPort)
		if isHTTPOnPort(host) {
			logrus.Warnf("Main KWOK cluster %s is serving HTTP on %s; recreating with HTTPS on this port", name, host)
			// Stop and delete, then recreate with secure port
			_ = exec.Command(km.kwokctlPath, "stop", "cluster", "--name", name).Run()
			_ = exec.Command(km.kwokctlPath, "delete", "cluster", "--name", name).Run()
			// Ensure disk state is gone to avoid kwokctl reusing insecure config
			_ = os.RemoveAll(clusterPath)
			// Fall through to secure create below
		} else {
	...
	// Verify HTTPS is actually served on the configured port
	host := fmt.Sprintf("127.0.0.1:%d", apiPort)
	if isHTTPOnPort(host) {
		return "", fmt.Errorf("post-create check: main cluster still serving HTTP on %s; please remove %s and retry", host, clusterPath)
	}
	return kubeconfigPath, nil
}

// isHTTPOnPort tries a quick HTTPS GET and inspects for the specific case where an HTTP server
// is bound on the given host:port. Returns true if HTTPS fails with an HTTP response.
func isHTTPOnPort(host string) bool {
	// Try HTTPS request to detect protocol mismatch
	...
	if err != nil {
		// Inspect error string for the common mismatch message
		if strings.Contains(strings.ToLower(err.Error()), "http response to https client") {
			return true
		}
		return false
	}
	// If we got a response, it's not HTTP-to-HTTPS mismatch; close and return false
	_ = resp.Body.Close()
	return false
}

// RehydrateFromDisk scans ~/.kwok/clusters for previously created KWOK clusters and
// reconstructs the in-memory mapping (by parsing names like "rancher-<clusterID>-<port>").
// It best-effort verifies connectivity and tries to start clusters that appear stopped.
func (km *KWOKClusterManager) RehydrateFromDisk() {
	clustersDir := filepath.Join(os.Getenv("HOME"), ".kwok", "clusters")
	...
	for _, e := range entries {
		...
		// Parse clusterID and port from name: rancher-<clusterID>-<port>
		lastDash := strings.LastIndex(name, "-")
		if lastDash <= len("rancher-") { continue }
		portStr := name[lastDash+1:]
		port, perr := strconv.Atoi(portStr)
		if perr != nil { continue }
		clusterID := name[len("rancher-"):lastDash]

		kubeconfigPath := filepath.Join(clustersDir, name, "kubeconfig.yaml")
		// Populate record
		km.clusterMutex.Lock()
		if _, exists := km.clusters[clusterID]; !exists {
			km.clusters[clusterID] = &KWOKCluster{
				Name:       name,
				ClusterID:  clusterID,
				Port:       port,
		...
		// Best-effort: make sure the API is reachable; attempt to start if not.
		if err := km.testClusterConnectivity(name); err != nil {
			logrus.Debugf("RehydrateFromDisk: cluster %s not reachable, attempting start: %v", name, err)
			startCmd := exec.Command(km.kwokctlPath, "start", "cluster", "--name", name)
			if out, serr := startCmd.CombinedOutput(); serr != nil {
				logrus.Debugf("RehydrateFromDisk: start failed for %s: %v, out=%s", name, serr, string(out))
			} else {
				_ = km.waitForClusterReady(name)
			}
		}
...
func (km *KWOKClusterManager) CleanupOldClusters(activeClusterIDs map[string]bool) error {
	...
	for _, entry := range entries {
		...
		// Only consider cleaning up per-tenant KWOK clusters created by this agent
		// These are named with prefix "rancher-<clusterID>-<port>". Never touch the main KWOK cluster
		// (e.g. "main-cluster") or any other non-prefixed directories.
		if !strings.HasPrefix(clusterName, "rancher-") {
			continue
		}

		// Check if this per-tenant KWOK cluster is still active
		isActive := false
